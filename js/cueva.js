...
function Plano3D(){
    ...
    this.getCasilla = (x, y, z ) => {
        ...
        if ( ! this.plano.hasOwnProperty("x" + x + "y" + y + "z" + z ) ){
            ...
            this.plano["x" + x + "y" + y + "z" + z] = 'rgba(0,0,0,0.1)'
        }
...
async function pintarandom(){

    eje = Math.floor(Math.random() * 3);
    masOMenosUno = Math.floor(Math.random() * 2);
    switch(masOMenosUno){
        case 0:
            //menos uno
            tablero.puntoDeVista[eje] -= 1
            break;
        case 1:
            //mas uno
            tablero.puntoDeVista[eje] += 1
